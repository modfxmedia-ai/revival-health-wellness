...
const FORM_ID = "n1YAHBwyND1PN2snQFzN";
const FORM_NAME = "\uD83D\uDD35 VERCEL Facebook Emsella LP";

/**
 * Popup dialog wrapping the LeadConnector (GoHighLevel) embedded lead
 * form for the Emsella ads landing page. Rendered via a portal to
 * document.body so `position: fixed` isn't constrained by any animated
 * (transformed) ancestor in the page tree — same approach as
 * components/lp/XerfLeadFormModal.tsx.
 *
 * On a successful form submission (detected via the LeadConnector iframe's
 * postMessage events), the user is routed to the Emsella thank-you page.
 */
export default function EmsellaLeadFormModal({
  open,
  onClose,
}: {
  open: boolean;
  ...
  useEffect(() => {
    ...
    const onMessage = (e: MessageEvent) => {
      ...
      if (isFormEvent && isSuccess) {
        onClose();
        router.push("/lp/emsella-offer/thank-you/");
      }
    };
  ...
  return createPortal(
    <>
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Free Emsella consultation request form"
        aria-hidden={!open}
        inert={!open}
